...
//Menemukan data student dengan menginputkan id nya, kemudian data nya akan di print
struct student_t find_student_by_id(struct student_t *_students,
                                    unsigned short int _student_size,
                                    char *_id) {
  struct student_t std;
  for(int x = 0; x < _student_size; x++){
    if(strcmp(_students[x].id, _id) == 0){
    printf("%s|%s|%s|%s|%.2f\n", _students[x].id, _students[x].name, _students[x].year, _students[x].study_program, _students[x].gpa);
  return std;
    }
  }
}

//Menemukan data course dengan menginputkan code dari course tersebut
struct course_t find_course_by_code(struct course_t *_courses,
                                    unsigned short int _course_size,
                                    char *_code) {
  struct course_t crs;
  for(int x = 0; x < _course_size; x++){
    if(strcmp(_courses[x].code, _code) == 0){
    char *text = grade_to_text(_courses[x].passing_grade);
    printf("%s|%s|%d|%s\n", _courses[x].code, _courses[x].name, _courses[x].credit, text);
    return crs;
      }
  }
}
